...
八层主线递进之后，设跨层收束章「路线选择与适用边界」（`docs/decision-boundary/`）。

该章不改变八层递进顺序，不新增 DDA 层。

它回答：何时不必建完整 Ontology、何时 Text-to-SQL 够用、何时 Data Loop 投入产出比不划算、自建与采购如何选型。

写该章前须已读完八层正文；各层正文可在工程实践段引用该章决策框架，但不得以该章替代各层定义。
