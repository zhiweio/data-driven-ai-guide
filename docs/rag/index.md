...
企业场景里，一个没有依据的答案是负债，不是资产。AI 输出必须可追溯、可引用、可校验。
...
检索增强生成（Retrieval-Augmented Generation，RAG）是企业知识基础设施的检索与落地层。前面建的知识基础设施（Knowledge Foundation，KF）提供了带绑定的知识资产，RAG 在其上做向量化、多路检索、视觉融合、接地、引用、评估，把知识变成 AI 可用的、有依据的答案。RAG 要解决的问题很具体：让 AI 在生成前接地（Grounding）到企业事实，生成后可引用（Citation）到具体来源，整个过程可评估（Evaluation）。

行业里常把"幻觉"当作 RAG 要消灭的敌人。在全书框架里，幻觉多是**语义传递不可靠**冒泡到生成端：企业事实没传到位，模型用通用语料拼出看似合理的答案。RAG 的价值因此不在"加一个向量库"，而在把知识产品的内容经检索、接地、引用，安全传到生成端。
...
"向量库加模型"最常见的坑是幻觉。模型没有企业知识，却会生成看起来合理的答案。问" savolitinib 的 Phase I 试验结果"，模型可能编造一组数据。没有接地机制，幻觉无法遏制。不要把幻觉只归因于模型——在多数企业场景里，根因是上游没有把可校验的企业事实稳定传到生成端。

等长切块还会把语义结构打碎。CSR 里"3.2 不良反应"的表格数据，被切成无结构文本块后，检索召回的是一句孤立描述，而不是表格所在章节的完整上下文。AI 看到叶子，看不到树。

视觉信息同样留不住。药企文档大量信息在表格、图表、分子结构图里。纯文本向量化把表格拍平成文字，把图表直接丢弃。AI 问"某化合物的剂量-响应曲线"，纯文本检索找不到图表，回答不了。

最后是没有引用与接地。模型即使答对了，也说不出依据。无法追溯的答案在企业场景不可用，因为没法校验、没法问责。模型在生成前没有对齐到企业事实，答案来自参数记忆，会过时、会混淆。

**RAG 不是"向量数据库 + 大模型"。** 那是 RAG 的一种最小实现，不是 RAG 的定位。RAG 的定位是企业知识基础设施的检索与落地层，向量检索只是检索的一种手段。把 RAG 等同于向量库加模型，会忽视知识组织、多路检索、视觉融合、接地、引用、评估这些真正决定企业可用性的部分。
...
DDA 方法重新看这一层：RAG 的核心不是检索，是让 AI 生成有依据。知识在进入检索前要按 Ontology 实体组织、按文档章节结构化、绑定版本，而非机械等长切块——语义结构化做不好，检索再先进也召回垃圾。召回侧也不止向量相似度：向量、关键词、图关系、Few-shot 示例多路融合，单一向量召回覆盖不了企业知识的全部形态。

文本与视觉用各自合适的编码器进入各自维度的向量空间，查询时跨空间并行召回再融合。不同模态、不同编码器的原始分数不可比，跨空间融合只用排名不用原始分。视觉信息（表格、图表、版式）要作为像素原生块保留并检索，与语义结构锚定，让 AI 能回答"图表里写了什么"这类问题。

生成前对齐企业事实，对齐了才生成，不对齐就拒答；生成后每条陈述挂上来源文档、版本、段落。评估也不是上线前一次性测试，而是运行时持续度量，结果回流驱动改进（这是下一章 Data Loop 的入口）。
...
评估要有基准才谈得上度量。垂直数据行业已经建了领域专用的评估基准：专利数据平台发布了专利 AI 基准测试集，覆盖新颖性检索、自由实施分析（FTO）、权利要求图表、专利翻译等八个子任务，样本来自真实审查与诉讼，系统无关（自带检索与模型即可评测）。这种基准的价值在于用真实业务场景定义"好"--不是通用的问答准确率，而是"新颖性检索的命中率""FTO 高风险漏检率"这类业务可直接判断的指标。对数据产品而言，评估基准是**质量门禁**；药企须自建监管场景黄金集，把化合物别名消歧、试验状态判断等写入 RAG 产品的验收标准——商业环境里 benchmark 是产品竞争力，企业内部则须自建同等纪律。
