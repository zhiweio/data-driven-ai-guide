...
为什么 AI 系统既要查数据，也要理解数据背后的知识？因为数据只记录"是什么"，知识解释"为什么"和"怎么做"。
...
数据工程师为什么要学知识基础设施（Knowledge Foundation，KF）？因为这是把结构化语义与非结构化知识统一起来的一层。前面建的数据本体（Ontology）和语义层（Semantic Layer，SL）解决了结构化数据的机器理解，但企业里大量知识是非结构化文档。KF 解决的问题是：把多类型、多模态的非结构化文档，变成统一组织、可绑定、可追溯、可版本化的知识资产，供上层检索增强生成（Retrieval-Augmented Generation，RAG）检索与接地。

语义传递不可靠不只发生在数仓表里，也发生在"文档库与数据各说各话"的边界上：数据侧叫 `NVP-001`，CSR 里写 `savolitinib`，SOP 按试验编号组织——没有绑定，AI 在结构化与非结构化两条传递链之间对不上号。KF 还要对付**语义时效不可靠**的另一面：SOP 换版了但文档库没标记过期，AI 引用的是两年前的规程。版本与时效管理因此是 KF 的一等公民，不只是存储问题。
...
第一，格式异构无法统一消费。PDF 的 CSR 用版面解析，Word 的 SOP 用段落解析，Excel 的不良反应清单是表格结构，图片的分子结构图是纯视觉，网页的法规是 HTML。没有统一的解析与组织，AI 要么只能处理单一格式，要么把所有文档当纯文本处理，丢掉结构。问"3.2 节的不良反应清单"，AI 在 PDF 正文里找不到，因为它在 Excel 附件里。

第二，视觉信息丢失。药企文档大量信息在表格、图表、分子结构图里。把 CSR 当纯文本处理，表格被拍平成乱序文字，图表被直接丢弃。AI 问"某化合物的剂量-响应曲线"，文档库里根本没有可消费的视觉信息。视觉信息不是装饰，是知识本身。

第三，知识与数据割裂。AI 查到化合物 `NVP-001` 的试验状态是 `terminated`，想解释为什么，得去文档库搜 `NVP-001`。但文档库里用的是通用名 `savolitinib`，不是 `NVP-001`。没有别名消歧的关联，AI 找不到对应文档。文档是孤岛，数据是另一个孤岛。这是传递链在结构化与非结构化之间的断裂——两边各自"传到了"，但传的不是同一实体的语义。

第四，知识没有版本与时效。两年前的 SOP 还在文档库里，AI 把它当作现行规程引用，给出过时答案。没有知识的时效管理，AI 的回答不可信。这是**语义时效不可靠**在知识侧的表现：不是检索不到，是检索到了已经过时的定义。
...
解读：左侧是企业知识库的真实形态--PDF、Word、Excel、图片、网页五类文档各自为政，AI 因格式杂乱、视觉丢失、无绑定、无版本四类原因无法消费。右侧 KF 通过多格式解析把它们统一成结构化知识，把视觉信息作为一等资产保留，绑定到 Ontology 实体，并做版本管理。这张图说明 KF 的价值不是"多存一份文档"，而是把杂乱的多模态资料统一成机器可消费的知识资产。

## 新的设计思想

DDA 方法重新看这一层：知识不能是杂乱文档库，必须被统一组织、绑定、版本化。KF 的设计思想有五点。

第一，多格式解析为统一结构。PDF、Word、Excel、图片、网页各有合适的解析适配器，但解析后都归一到同一种结构化表示：章节树加类型化块。不论来源格式，知识资产在 KF 里形态统一。

第二，视觉资产是一等公民。表格、图表、分子结构图不丢弃、不拍平，作为与文本块并列的视觉资产保留。视觉信息是知识本身，不是文本的附属。

第三，知识实体绑定 [^1]。每份知识资产（文本块或视觉资产）显式绑定到 Ontology 实体。SOP 绑定到试验实体，CSR 绑定到化合物与试验实体。知识不再悬空，而是锚定在业务对象上。

第四，知识版本与时效。知识有版本，有过期机制。现行规程与历史规程可区分，AI 不会引用过时知识。

第五，引用基础设施。每份知识资产带完整的溯源坐标（路径、文档 ID、页码、块 ID），AI 引用时能追溯到具体位置。引用是问责的基础。
...
解读：五类文档经格式适配器路由，统一进入语义结构化，产出文本块与视觉资产两类一等公民。两者都绑定到 Ontology 实体，经版本与时效管理，带上溯源坐标后供 RAG 层检索与引用。这张图说明 KF 是多格式到统一结构、多模态到可绑定资产的转换层，视觉资产与文本块同等地位，不丢弃不拍平。
...
解读：任意格式文档经标题检测重建章节树，产出三类类型化块：文本块、图块、表块。每个块带元数据：`path` 是层级位置（如 `{kb}/{file}/{3 试验结果}/{3.2 不良反应}`），`summary` 是摘要，`keywords` 是关键词，`connect_to` 链接到块内嵌入的图或表。这张图说明语义结构化把杂乱文档还原成可导航的语义树，每个块都知道自己的类型与在树里的位置，为后续绑定与检索提供结构基础。
