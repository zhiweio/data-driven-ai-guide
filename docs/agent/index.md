...
解读：Agent 依次经 SL 取化合物与试验数据，经 RAG 取现行 SOP，RAG 又向 KF 取绑定到试验实体的规程与 CSR。每一步都有结构化记录，每一步的工具调用都经语义层，不直连裸库。最终输出带引用与合规判断。时序图与上文消费链路架构一致：可靠性来自第 2–5 章已加固的传递链，不是模型本身。
...
解读：硬实时场景两者都不合适，应预计算或规则引擎。选 Agent 之后，仍须遵守本章消费链——Workflow 也不应旁路 SL 直连裸库。
