...
药明诺华的 AI 项目会上三套方案：AI 产品团队要两周内上线聊天 Demo；数据平台团队主张先补数据契约与语义层；采购部门在评估 Palantir Foundry 一站式平台。三个方向都不全错，但若同时推进，会得到三套口径、三个影子产品目录。本章要回答的不是「哪套技术更先进」，而是：**在资源有限时，DDA 八层应建到哪一档、哪些层可暂缓、自建与采购如何组合。**

读完前八章，你已看到完整主线：从第 1 章数据契约到第 8 章 Ontology Evolution 闭合回路。本章是收束——在「驯服语义不可靠」与「交付升级版数据产品」两条主线之上，补上诚实的适用边界：资源有限时，不必每层都上满；选型错误与不上线同样危险。

## 问题

为什么企业不能每层都上满 DDA？因为每一层都是持续投入：契约维护、Ontology 共建、SL 版本化、RAG 黄金集、Data Loop 回流审批。误用全套 DDA——在 PoC 阶段就建完整 Ontology 与全链路 Data Loop——与误用 Text-to-SQL 或另建 AI 沙箱同样危险。

数据工程师为什么要学这一章？因为你是路线的主要执行方。业务方和 AI 产品团队往往默认「上了大模型就能问一切」；采购方往往默认「买了平台就 Ontology 齐全」。你需要一套可辩护的分阶段路径：先加固传递链的哪一段，再对付时效的哪一环，什么情况下可以诚实地说「这一层现在不必建」。

本章解决的问题是：在两类语义不可靠（传递 vs 时效）的诊断下，选择轻量、标准或完整三档建设路径，并判断自建、采购或混合组合。
...
两类极端的失效机制不同，但根因一致：**没有先诊断语义不可靠的类型与严重度，就选了与问题不匹配的路线。**

沙箱失效，因为绕开了数据产品目录，在组织内再造未受契约约束的影子产品。采购空心化失效，因为把「平台能力」当成「数据前提」——Ontology 绑定的是数据资产，资产本身若不 AI 就绪，平台只多一层空壳。

过早全套 DDA 失效，因为维护成本超过业务价值：为 MeSH 已充分覆盖的通用医学概念建完整规则层，投入大、收益低（FAOS 反参数化知识效应，见[第 2 章](../ontology/index.md) FAOS 小节）。过晚建 SL 失效，因为第 6 章 Agent 已直连裸库，技术债固化后再收口径，成本倍增。

过早 Data Loop 失效，因为无生产流量、无黄金集、无回流权责时，反馈管道是空转；组织会把它当成「又一个 ETL」，然后放弃。
...
解读：四种误选表面不同，根因都是未先分型诊断。沙箱与永久 Text-to-SQL 主要加剧传递不可靠；采购空心 Ontology 同时恶化传递与时效；过早全套 DDA 常在未验证高价值域前就铺开维护成本。读各章时可用本章决策流对照：你的主要症状是「对不上」还是「过时了」？
...
解读：决策流从症状分型入手，而非从工具入手。传递问题看人复核还是机器自主消费；时效问题看是否具备回流前提。L2 是多数企业的务实目标；L3 需要组织承诺。写回源系统（终止试验触发状态变更、写审计日志）是采购 Palantir/Fabric 操作层的常见触发条件，不是「要更聪明的 KG」。

## 工程实践

### 反面教材一：何时 Ontology 是过度工程

FAOS 框架（2025 年公开，仅为参考）观察到反参数化知识效应：在模型已熟知的概念上建 Ontology，收益有限甚至可能干扰；在模型知识稀疏的企业特定概念上建 Ontology，收益显著。
...
**混合常见形态：** 数据契约与 SL 自建（dbt MetricFlow 或 Cube），Ontology 操作层用平台 Actions 处理写回；或实体定义自建 YAML，绑定到 Fabric IQ Bindings 做联邦查询。关键是不论采购与否，**数据产品目录与单一口径须先存在**。

### 中小团队路径（无大型实体对齐积累）

不必等「三亿企业实体对齐」级工程积累再起步：
...
读完本章后，可用[附录](../appendix/index.md)中的成熟度模型、成本估算与 RACI 表把决策落到人月与组织分工；各层正文中的业界路线对照（第 2、3、6 章）应在本章决策框架内理解，而非单独选型。
...
**自检（依据 WRITING_STYLE §9）**：本章为何需要？资源有限时不能每层上满。数据工程师为何关心？路线执行与跨部门辩护。解决什么问题？分档建设 + 自建采购选型 + 反面教材。属哪一层？跨层决策收束。改变什么架构？从「默认全套 DDA」改为「诊断后分档」。
