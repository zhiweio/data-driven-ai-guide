...
AI 时代的变化是：这些 API 从"给人的程序调用"变成"给 Agent 调用"，从 REST 接口变成模型上下文协议（Model Context Protocol，MCP）[^4] 工具。SL 暴露为 MCP Server 后，Agent 能像调用内置工具一样调用语义接口，不需要把 SQL 拼进提示词。两家公司都已把数据能力封装为 MCP Server 对外开放。语义层做的事，是把数据产品的交付接口**升级为 Agent 原生**——产品还是那些产品，通道从 REST 扩到 MCP。MCP 是通道，不是 SL 的定义。

企业转型的关键不是先上 MCP，而是**先让内部能力达到数据产品级**——每个对外暴露的能力都是语义接口，有单一口径、有版本，而非裸表。启信宝、智慧芽在已有数百个数据 API 的产品目录上叠加 Agent 能力；多数企业若跳过产品化、让 Agent 直连裸库或自建向量库语义，等于新产品消费者接入了一个未完工的产品目录。
