...
Ontology 定义了业务世界是什么意思，但 Agent 不能直接读 YAML 实体定义。语义层是 Ontology 到消费方之间的**传递通道**：把世界模型工程化为可调用的语义接口，在物理 schema 变动与多消费方并存时，保证传下去的口径一致。口径漂移、字段改名即崩、RAG 与 Agent 各说各话，都是传递不可靠在消费边界上的具体症状。
