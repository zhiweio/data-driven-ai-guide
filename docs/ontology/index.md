...
通用大语言模型（Large Language Model，LLM）的预训练语料里有互联网常识，没有你企业的业务规则。它不知道化合物 `NVP-001` 和通用名 `savolitinib` 是同一个药，不知道 c-Met 抑制剂的临床试验要遵循 CDISC SDTM [^3] 标准，也不知道"活性化合物"在药明诺华研发流程里特指通过初筛的候选物。这些共识散落在不同系统、不同人脑里。

对数据工程师来说，Ontology 是机器理解企业业务世界的方式。前一章建好的 AI 就绪数据平台提供了"原材料"，原材料本身不携带"如何理解"的说明。Ontology 补上这份说明：显式定义业务世界里有哪些实体、实体间有什么关系、遵循什么规则、受什么约束。没有它，AI 只能凭模型自己的猜测理解业务——在受监管的药企环境里，这不可接受。

Ontology 给 AI 一份与人类业务共识对齐的世界模型[^7]，让判断有据可依，而不是凭模型记忆发挥。承接上一章与前言：Ontology 是数据产品的**语义说明书**，在业务层建立语义传递的单一来源，把散落的企业共识收拢为机器可读的实体、关系、规则与约束。没有这一层，下游只能各自猜口径。
...
AI 读不到散落的语义。用全书框架说，这是**语义传递不可靠**在本体层的集中爆发：数据进来了，但"这个字段、这个编号、这个状态在业务上是什么意思"没有单一、机器可读的答案。

同一个"活性化合物"，研发系统、临床系统、BI 报表各定义一遍，定义之间还有细微出入。AI 不知道该信哪一个。Agent 被问"有多少活性化合物"时，不同口径给出不同答案，无法追责。

企业规则也进不了机器。"Phase I 试验失败则该化合物的研发状态自动转为 suspended"写在研发流程文档里，不在任何数据系统里。AI 无法执行它读不到的规则。

别名消歧同样缺依据。`NVP-001`、`savolitinib`、`AZD6094`、`HMPL-504`、`volitinib` 指向同一个 c-Met 抑制剂，模型不知道。靠模型猜，召回率上不去；靠人工对齐，不可持续。这是典型的"机器读不到的语义"。
...
图：散落语义与 Ontology 单一来源的对比（DDA 层：Ontology）。左侧语义散在表名、字段、注释、BI 口径四处；右侧 Ontology 把实体、关系、规则收拢为单一来源，Agent 经此理解业务世界。

## 新的设计思想

DDA 方法重新看这一层：要让机器理解企业业务世界，不能靠模型猜测，必须显式定义。Ontology 就是这份显式定义。

它定义四样东西：实体（Entity，业务对象，如化合物、靶点、试验）、关系（Relation，实体间的业务连接，如"化合物作用于靶点""试验研究化合物"）、规则（业务规则，如"Phase I 失败则状态转 suspended"）、约束（业务约束，如"同一化合物的研发代号全局唯一"）。

**Ontology 不是知识图谱（Knowledge Graph，KG）[^1]，不是 RDF[^8]，不是 OWL[^9]，不是图数据库的产品特性。** KG 是图结构的实现技术，RDF[^8] 与 OWL[^9] 是具体的表示语言，图数据库是存储引擎。它们可以是 Ontology 的实现方式之一，但不是 Ontology 本身。Ontology 回答"机器如何理解业务世界"；KG 是其中一种落地形态。本书谈方法论时用 Ontology，谈具体图存储实现时才用 KG。

数据模型描述数据怎么存，KG 描述知识怎么检索，Ontology 描述业务世界是什么意思、遵循什么规则、支持什么操作。三者层次不同，不能互相替代。
...
图：Ontology 四层架构（DDA 层：Ontology）。实体层定义业务对象，术语层处理别名与标准编码，规则层编码业务规则与约束，动作层定义可执行操作；实体层向下绑定 AI 就绪数据平台的数据资产，Agent 经动作层消费 Ontology，而非直接读原始数据。
...
`aliases` 用行业标准编码把同一化合物的多个名称对齐：研发代号、通用名、CAS 号、PubChem CID、ChEMBL ID、DrugBank ID、UMLS CUI。`standard_codes` 进一步接入 RxNorm、MeSH、CDISC SDTM 等行业标准。Agent 从哪个系统的哪个名称出发，都能落到同一个实体上——机器可读的别名消歧，不依赖模型猜测。
...
本书默认路径：第 1 章契约提供 L1 绑定，本章 YAML 实体与三层治理提供 L2/L3，图结构可选作关系层实现。落地时企业常问：**这套描述层自建，还是采购 Palantir / Fabric IQ？** 对照的不是厂商功能清单，而是能否满足上文四要素（实体/关系/规则/约束）并绑定 `compound_active`。下一章语义层与第 6 章 Agent 只消费已绑定的语义，不能替 Ontology 补别名。
...
图：Ontology 描述层与操作层 — 自建 vs 平台采购（DDA 层：Ontology）。自建 YAML 覆盖描述层即可支撑「查询理解」类 Agent；终止试验同步 ERP 须评估操作层（平台 Actions 或自建写回）。GraphRAG / LazyGraphRAG 属第 5 章 RAG 检索实现，不能替代本章实体与规则的单一定义。

### 落地深潜：Palantir Foundry Ontology 与药明诺华

对照表里的 **Palantir Foundry Ontology** 行，用一条完整业务链拆开看，否则容易把它当成「带写回功能的图数据库」，或误以为买了平台就不用做本章的别名与规则共建。Palantir 官方把 Ontology 定义为组织的**操作层（operational layer）**：它坐在已接入平台的数据集、虚拟表与模型之上，把数字资产映射到真实世界的对应物（工厂、订单、**化合物、临床试验**），同时承载**语义元素**（对象、属性、链接）与**动力学元素**（Actions、Functions、动态权限）。它不是语义层的替代品，也不是「又一个数据目录」，而是把「业务世界是什么意思」与「业务世界允许怎么改」绑在同一套可审计对象模型上。

#### 场景：Phase I 失败，必须终止试验并同步全公司口径

药明诺华临床运营经理收到邮件：`NCT04012345`（`NVP-001` / savolitinib 的 c-Met 抑制剂 Phase I 试验）因安全性信号需按 SOP `SOP-CT-012` 终止。传统做法：在 CTMS 改试验状态、在研发管线表把化合物标为 `suspended`、通知监管事务准备递交说明、在 Slack 里口头同步 BI 团队「这药暂时别算在研」。四处改动、四个口径、邮件与截图当审计证据——企业规则无法编码、别名无法机器对齐、写回无法追责，叠在一起。
...
**Object Type 不是表名翻译**。`dwd_compound` 是数据资产；`Compound` 对象类型声明「业务上什么叫一个化合物、哪些属性代表研发状态、链到哪些试验」，并通过 backing dataset 与契约字段对齐。这与本章「实体层向下绑定 AI 就绪数据平台」一致。Foundry 把绑定与索引做成平台能力，但药明诺华仍须与临床、研发共建 `status` 枚举与终止 SOP。

**Link Type** 处理多跳业务。临床经理在 Object Explorer 里从 `Compound: NVP-001` 沿 `studies` 链到 `Trial: NCT04012345`，再沿 `conducted_at` 到研究中心、沿 `competes_with` 看同靶点竞品管线——上文「化合物 → 试验 → 研究者 → 竞争管线」图结构的消费界面。图存储是实现，Link Type 是 Ontology 上的关系契约。

**Action Type** 是 Foundry 相对「纯描述型 Ontology」的增量，也是对照表里「操作层」一词的来源。Action 是一次事务，按用户定义的逻辑修改一个或多个对象的属性与链接，并可附带副作用（通知、Webhook、触发管道构建）。`TerminateTrial` 可设计为：
...
图：Foundry Action 写回链 — Ontology 作变更枢纽（DDA 层：Ontology → 数据基座 → Semantic Layer）。终止 Action 更新对象并物化 writeback，经管道刷新契约表与语义层在研口径。
...
**Functions**（同为动力学元素）适合复杂逻辑。按 FAOS 建议，不在通用医学概念上铺规则，而用 Function 计算「该化合物是否满足内部暂停管线规则」，再被 Action 或 Workshop 按钮调用，与本章 L3 规则层「优先模型知识稀疏区」一致。

#### 解决了什么问题：相对自建 YAML 与相对裸平台

相对散落语义，Foundry Ontology 把实体、别名、链接、可执行终止流程收进单一操作层，Agent 与人工共用同一对象 ID 与同一 Action 审计日志。相对本章自建 YAML，采购路线用 Object/Link/Action 产品化封装版本、权限、写回物化、跨应用一致编辑，适合终止试验、管线状态机、跨部门写回强审计的场景。但它不自动解决：`NVP-001` 与 savolitinib 的别名仍需人定义；「在研」是否含 `preclinical` 仍须在 Ontology 与第 3 章 SL 同源；SOP 正文仍在第 4 章 KF，Ontology 只编码可执行状态机，不存 PDF 全文。
...
**接受演进**：Ontology 会随业务变化而演进（下一章 Ontology Evolution 的主题）。不要追求一次建模永久不变。
...
**自检（依据 WRITING_STYLE §9）**：AI 需要与业务共识对齐的世界模型；Ontology 是全书两大核心之一，是机器理解业务世界的方式；解决 AI 靠猜测理解企业业务的问题；属 Ontology 层；把散落语义收拢为机器可读的单一来源，并绑定数据资产。
