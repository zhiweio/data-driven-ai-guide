...
药明诺华的做法是：新通用名出现时，先在行业标准编码库（UMLS、RxNorm）查是否已有登记，有则直接引用标准编码，无则内部登记并标注待标准化。这样别名消歧规则随新名出现持续演进，又不脱离标准体系。垂直数据行业对权利人、专利族规则的持续版本化，与上述纪律同构（见[附录](../appendix/index.md)）——Ontology Evolution 本质是数据产品目录的**版本管理**，须有变更记录、下游影响评估与历史兼容。
...
本章闭合了全书主线：第 7 章 Data Loop 的反馈在此落地为版本化演进。但「是否每次变更都须建满四层 Ontology」「中小团队先演进哪一块」属于投入产出问题，不在本章展开——见收束章[路线选择与适用边界](../decision-boundary/index.md)，以及[附录](../appendix/index.md)成熟度模型与 RACI。
