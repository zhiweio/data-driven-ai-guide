...
药明诺华那套系统刚上线时，化合物别名映射是对的，试验状态口径是对的，SOP 引用是现行版。三个月后，新化合物进了管线，新通用名出现了，SOP 换了版本，试验状态流转规则改了。系统若不能持续吸收这些变化，答案就开始偏离现实。有人把这叫"第三月停滞"：上线初期表现不错，几个月后因未跟上业务变化而停滞乃至衰退。不是模型退化，是语义时效在时间维度上的累积漂移——当时对齐的业务共识，过一阵就落后了。

前面各层解决"AI 如何消费数据"；Data Loop 解决"AI 如何反过来修正数据"。没有它，AI 系统是一次性产物，上线即开始衰退；有了它，才谈得上持续进化。全书两大核心之一，也是 AI 原生（AI Native）企业最难复制的护城河——本质是数据工程问题，不是模型问题。

当业务变化而定义滞后时，靠反馈把数据、知识与 Ontology 拉回与现实对齐。这是数据工程师最能发挥价值的地方。
...
单向管道没有回流，错误与变化无法被系统吸收。

Agent 答错了，错误停在用户那里，不回流到数据层。下次同样的问题，同样错。化合物别名映射错了，没人修，一直错下去。

SOP 换版了，知识基础设施（Knowledge Foundation，KF）里的现行版本没跟着换，RAG 还是引用老版本。AI 给出过时答案，因为知识源过时了。

新化合物类别进了管线，Ontology 里没有对应实体定义，Agent 不认识。没有机制让业务变化反映到 Ontology。

**数据闭环不是新的 ETL，不是数据管道的同义词，不是模型再训练流程。** ETL 是单向搬运，Data Loop 是双向反馈修正；模型再训练只动参数，Data Loop 修正的是数据、知识、Ontology。把它等同于 ETL 或再训练，就错过了护城河的本质。
...
反馈分三路回流——数据、知识、Ontology（虚线自环表示本体演进）。回流方向与正向消费相反：AI 输出反向修正数据层，不是又一条管道。

## 新的设计思想

DDA 方法重新看这一层：数据系统必须是双向的，AI 输出是改进数据层的输入。

三个环节：

**反馈采集**——把 Agent 执行结果、用户纠错、评估判定收进来，作为回流原料。显式信号（用户点"答错了"）强但稀疏；隐式信号（反复追问、重新表述）弱但密集，往往更有价值。

**反馈回流**——分三路：修正数据（如别名映射错误）、修正知识（如 SOP 现行版本、RAG 检索参数）、修正 Ontology（如新增化合物类别）。每路有校验与审批，不能让错误反馈污染数据层。RAG 自身的评估闭环（检索/融合/生成质量）是"修正知识"在 RAG 层的具体形态。

**可观测闭环**——能看到反馈在哪里产生、回流到哪里、改进了什么。没有可观测，就不知道闭环在不在转。

模型可以换，提示词可以抄；运转良好的数据闭环是长期工程积累，竞争对手短期建不起来。
...
上半段是正向消费链；下半段是可观测→评估→反馈采集→三路回流。RAG 节点自带评估回流，是「修正知识」的内部闭环。Data Loop 不是独立组件，是把可观测与评估接入消费链形成的闭环。
...
要把飞轮转起来，闭环得真正接通：反馈采得到、回流修得对、改进能验证。任何一环断掉，飞轮就停。
